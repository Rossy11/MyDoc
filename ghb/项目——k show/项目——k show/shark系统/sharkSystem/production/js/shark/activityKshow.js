 ...
 */
angular.module('sharkApp', []).controller('sharkCtrl', function ($scope, $compile, $timeout, $http) {
        //search
        ...
        var midPress = -1;//记录上次点击的条目索引
        var IsSelect = false;//当前是否是单个检索
        var countMach = 0;//记录选中的mid数量
        ...
        // secrectKey="883c1415d9000f2c517eb8aafed87225";
        var jsonDatas = {
            count: 50,
            page: 1,
            IsActivity: 1
        ...
        function getMachineList(jsonDatas, secrectKey, str) {
            ...
            $http({
                ...
                var jsonData = deCodeData(res);
                $scope.checkAllInfo = false;
                if (str == "turnPage" || str == "select" || str == "mids") {//当不为翻页时，初始化管理弹框和页面表格
                ...
                //$scope.machineInfo = resData;
                if (notNull(resData)) {
                    if (str == "mids") {
                        resData.unshift($scope.machineInfo[midPress]);
                        $scope.MidListPop = resData;
                        //log($scope.MidListPop)
                        $scope.popMidS = true;
                    } else {
                        $scope.checkAllInfo = false;
        ...
        $scope.check = function (id) { // 当所有都选中时
            ...
            if ($scope.machineInfo[id].box) {
                ...
                if (machineSave.length != 0 && !IsSelect) {
                    countMach += 1;
                }
                if ($scope.machineInfo.length == 1 && $scope.machineInfo[id].box) {
                    $scope.checkAllInfo = true;
                } else if (machineSave.length != 0 && countMach == $scope.machineInfo.length) {
                    $scope.checkAllInfo = true;
                } else if (machineSave.length == 0 && $scope.totalCount == $scope.machineInfo.length) {
                    $scope.checkAllInfo = true;
                }
            } else {
                $scope.totalCount -= 1;
        ...
        $scope.checkAll = function ($event) {
            ...
            if ($scope.checkAllInfo) {
                ...
                if ($scope.machineInfo.length == 1) {
                    $scope.handleList.push($scope.machineInfo[0]);
                } else {
                    $scope.handleList = setHandleList($scope.machineInfo, $scope.handleList);
                }
            } else {
                $scope.totalCount -= $scope.machineInfo.length;
                if ($scope.machineInfo.length == 1) {
                    $scope.handleList.remove($scope.machineInfo[0]);
                } else {
                    $scope.handleList = removeNow($scope.machineInfo, $scope.handleList)
                }
            }
            $scope.totalCount = $scope.handleList.length;
        };
        var addList = [];
        var staList = [];
        var verList = [];
        ...
        var reList = [];
        // 检索确认
        $scope.submitSelect = function (id, sub) {
            selectCheck(id, sub);
            if (sub == "address") {
                addList = [];
        ...
        $scope.popSign = function ($event) {
            ...
            $scope.popMidS = false;
            if (midPress != -1) {
                document.getElementById("hintImg" + midPress).style.background = "url(image/hintPress.png) no-repeat center";
            }
            $scope.handleDiv = true;
        ...
        $scope.bodyClick = function () {
            ...
            // alert($scope.handleDiv)隐藏管理，一级弹框
            if ($scope.handleDiv) {
                $scope.handleDiv = false;
            }
        }
        ...
        //显示组Mid弹框
        $scope.showMidS = function (index, event) {
            if ($scope.popMidS && midPress == index) {
                $scope.popMidS = false;
                document.getElementById("hintImg" + midPress).style.background = "url(image/hintPress.png) no-repeat center";
            } else {
                //$scope.popMidS = true;
                ...
                var TH = 100;//midTable（MidSPop）的高度
                if (setInitImg()) {
                    if (bodyH - CY - 20 > TH) {
                        document.getElementById("MidSPop").style.top = (CY - 55) + "px";
                        document.getElementById("hintImg" + midPress).style.background = "url(image/hintDown.png) no-repeat center";
                    } else {
                        document.getElementById("MidSPop").style.top = (CY - 80 - 100) + "px";
                        document.getElementById("hintImg" + midPress).style.background = "url(image/hintUp.png) no-repeat center";
                    }
                }
                var submid = $scope.machineInfo[index].submid;
                var jsonDatas = {
                    count: 50,
                    page: 1,
                    submid: submid
                ...
                log("$$$$$$$$$$$$$$");
                log(jsonDatas);
                getMachineList(jsonDatas, secrectKey, "mids");
            }
        }
        //屏幕尺寸变化时，隐藏Mid表
        ...
        function setInitImg() {
            for (var i = 0; i < $scope.machineInfo.length; i++) {
                document.getElementById("hintImg" + i).style.background = "url(image/hintPress.png) no-repeat center";
            }
            return true
        }
    }
