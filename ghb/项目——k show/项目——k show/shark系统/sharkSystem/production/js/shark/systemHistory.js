...
app.controller('sharkCtrl', function ($scope, $compile, $timeout, $http) {
    ...
    getSelectList(secrectKey, "update");
    //升级记录,标签记录
    $scope.chooseSelect = function (str, select) {
        $scope.historyShow = true;
        if (str == "update") {
            $scope.updateHis = select;
            $scope.signHis = "标签记录";
            upList = [];
            siList = [];
            angular.forEach(updateListOld, function (v, k) {
                if (formatDateStr(v.info) == select) {
                    upList.push(v.info);
            ...
            $scope.signHis = select;
            $scope.updateHis = "升级记录";
            upList = [];
            siList = [];
            siList.push(select);
            getMachineList(handleParam(1), secrectKey, "all");
    ...
    function formatDate(list) {
        ...
        angular.forEach(list, function (v, k) {
            json.info = formatDateStr(v.info);
            json.box = false;
            listNew.push(json);
            json = {};
        });
    ...
    function formatDateStr(str) {
        //str = str.substring(0, 10) + "            " + str.substring(12, 16)
        return str;
    }
    ...
    function setListCheck(list, str) {
        ...
        if (str == "address") {
            ...
            updateListOld = listA;
            $scope.updateList = formatDate(listA);
        }
        return listA;
    }
