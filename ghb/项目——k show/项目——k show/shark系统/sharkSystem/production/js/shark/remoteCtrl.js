...
angular.module('sharkApp', []).controller('sharkCtrl', function ($scope, $compile, $timeout, $http) {
    ...
    $scope.prePage = function () {
        if (pageSelect > 1) {
            pageSelect -= 1;
            getMachineList(handleParam(pageSelect), secrectKey, "turnPage");
        }
    }
    ...
    function getMachineList(jsonDatas, secrectKey, str) {
        ...
        $http({
            ...
            if (notNull(resData)) {
                ...
                setCheck(jsonData.machines, str);
            } else {
    ...
    $scope.selectChange = function () {
        if (regx.exec($scope.search)) {
            ...
            initAllCheck($scope.machineInfo);//删除单个搜索框信息之后，初始化表格信息,初始化全选勾选
        } else {
